...
def train_and_evaluate_lstm(input_csv, model_save_path, scaler_save_path, num_epochs=60, batch_size=32, test_size=0.2, random_state=42):
    ...
    y = data[['Wind Speed']].values  # Target for LSTM
    X = data.drop(['Wind Speed'], axis=1).values
    ...
    loss_df.to_csv('training_losses_ws.csv', index=False)
    ...
    metrics_df.to_csv('evaluation_metrics_ws.csv', index=False)
...
def predict(model_path, scaler_path, input_csv, output_csv, plot_path=None):
    ...
    # Extract features (X) for prediction and drop 'Wind Speed' (or relevant feature)
    ...
    if plot_path:
        ...
        plt.plot(time_in_days, predictions, label='Predicted Wind Speed')
